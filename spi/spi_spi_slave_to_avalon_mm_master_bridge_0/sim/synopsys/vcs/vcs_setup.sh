...
TOP_LEVEL_NAME="spi_spi_slave_to_avalon_mm_master_bridge_0"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [spi_spi_slave_to_avalon_mm_master_bridge_0::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {spi_spi_slave_to_avalon_mm_master_bridge_0::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [spi_spi_slave_to_avalon_mm_master_bridge_0::get_design_files "$QSYS_SIMDIR"]]
...
vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  ...
  $QUARTUS_INSTALL_DIR/eda/sim_lib/synopsys/tennm_hssi_atoms_ncrypt.sv \
